...
wav_sing_root = '/Users/keir/Documents/s2let_ilc_planck/deconv_data/s2let_ilc_dir_hypatia_memeff_planck_deconv_tapered_wav_3999_1dot2_25_1_j'
...
#cbar = plt.colorbar(cax,ticks=[-1.*LIM,0,1.*LIM],shrink=0.5) #,orientation='vertical')
...
fig, axs = plt.subplots(1,1) #jmax-jmin+1, ndir, figsize=(4*ndir, 3*(jmax-jmin)))
#axs = axs.ravel()
# Loop through scales j and directions n
for j in range(46,47): #jmin, jmax+1):
    for n in range(0, ndir):
        #Loading single wavelet maps
        wav_sing_fits = wav_sing_root + str(j) + '_n' + str(n+1) + '.npy'
        wav_maps = np.load(wav_sing_fits)
        ...
        print bandlimit
        # Make the plot!
        myplot(wav_maps, bandlimit, axs,plt,title='Scale '+str(j)+'/'+str(jmax)+', direction '+str(n+1)+'/'+str(ndir)) #(j-jmin)*ndir+n],plt,title='Scale '+str(j)+'/'+str(jmax)+', direction '+str(n+1)+'/'+str(ndir))
        del wav_maps
        #fig.colorbar(axs[(j-jmin)*ndir+n],ticks=[min(wav_maps[offset:offset+nelem]),max(wav_maps[offset:offset+nelem])])
